...
@WebServlet("/AccessGroupServlet")
public class AccessGroupServlet extends HttpServlet {
	private static final long serialVersionUID = 1L;
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		String method=request.getParameter("method");
		switch(method){
			case "getOfficerTypes" :	List<OfficerType> officertypes=OfficerTypeDAO.getAllOfficerTypes();
			...
			case "getAll"	:			int node = Integer.parseInt(request.getParameter("node"));
										List<Menu> menus = MenuDAO.getAllMenuByNode(node);
	...
	public static String addAccessibleMenu(HttpServletRequest request){
		...
		try {
			...
			result = AccessGroupDAO.addAccessibleMenu(accessibleMenus);
	...
	public static String modifyAccessibleMenu(HttpServletRequest request){
		...
		try {
			...
			result = AccessGroupDAO.modifyAccessibleMenu(accessibleMenus);
		} catch (NumberFormatException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		} catch (JSONException e) {
