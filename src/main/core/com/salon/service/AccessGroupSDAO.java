		...
		if(accessibleMenus.size()==0){
			...
			try {
				for (AccessibleMenu accessibleMenu : accessibleMenus) {
					PreparedStatement query = con.prepareStatement("INSERT INTO accessiblemenus (menuid,officertypeid,nodeid) VALUES (?,?,?)");
	...
	public static String modifyAccessibleMenu(List<AccessibleMenu> accessibleMenus){
		...
		if(accessibleMenus.size()==0){
			...
			try {
				...
				PreparedStatement queryDel = con.prepareStatement("DELETE  FROM accessiblemenus WHERE nodeid=? AND officertypeid=?");
