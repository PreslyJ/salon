...
public class MenuDAO {
	...
	public static List<Menu> getAllMenuByNode(int node) {
		...
		try {	 
			PreparedStatement query=con.prepareStatement("SELECT * FROM menu WHERE node=?");
			query.setInt(1, node);
			query.executeQuery();
	...
	public static List<AccessibleMenu> getAllMenuByNodeNOfficer(int node,int officerTypeId) {
		...
		con=JDBCConnection.getConnection();
		try {	 
			PreparedStatement query=con.prepareStatement("SELECT * FROM accessiblemenu WHERE nodeid=? AND officertypeid=?");
			query.setInt(1, node);
	...
	public static List<Menu> getAllMenuByOfficer(int officerId) {
		...
		try {	 
			...
			while (rs.next()) {
				...
				PreparedStatement queryMenu=con.prepareStatement("SELECT * FROM menu WHERE menuid=?");
