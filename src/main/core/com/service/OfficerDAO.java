...
public class OfficerDAO {
	public static String addUser(String titleId,String firstName,String surname,String mobileNo,String officerTypeId,String userName,String password,String status,int createdUserId){
		...
		if(titleId=="" || firstName==""|| surname==""||mobileNo==""||officerTypeId=="" ||userName==""||password==""){
			...
			try {	 
					...
					PreparedStatement query=con.prepareStatement("SELECT * FROM officer WHERE username=? ");
					...
					if(result.next()){
						...
						query = con.prepareStatement("INSERT INTO officer (titleid,firstname,surname,officertypeid,username,password,status,createduser,createddate,mobileno) VALUES (?,?,?,?,?,?,?,?,?,?)");
	...
	public static List<Officer> getAllOfficers(){
		...
		try {	 
			...
			PreparedStatement query=con.prepareStatement("SELECT * FROM officer ");	
			query.executeQuery();
	...
	public static String getFullNameByOfficerId(int id){
		...
		try {	 
			...
			PreparedStatement query=con.prepareStatement("SELECT firstname FROM officer where officerid=?");
			query.setInt(1, id);
			query.executeQuery();
			ResultSet result=query.getResultSet();     			
			while (result.next()) {
	...
	public static int getOfficerIdByName(String name){
		...
		con=JDBCConnection.getConnection();
		try {	 
			PreparedStatement query=con.prepareStatement("SELECT officerid FROM officer  WHERE  firstname ilike ?");
