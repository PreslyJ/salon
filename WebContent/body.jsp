	...
	$.getAll=function(){
		...
		$.ajax({
			...
			success : function(data){
				...
				var fieldData3=new Array();
				fieldData3=data["menu3"];
				var listCal = document.getElementById("stockIn");
				for( var i =  0 ; i < fieldData3.length ; i++){
                    listCal.innerHTML = listCal.innerHTML + "<a class='list'  id='"+fieldData3[i][1]+"'>"+fieldData3[i][0]+"</a>";
            	}
				
				
				var fieldData4=new Array();
				fieldData4=data["menu4"];
				var listAutho = document.getElementById("stockOut");
				for( var i =  0 ; i < fieldData4.length ; i++){
					listAutho.innerHTML = listAutho.innerHTML + "<a class='list'  id='"+fieldData4[i][1]+"'>"+fieldData4[i][0]+"</a>";
            	}
				
				
				var fieldData5=new Array();
				fieldData5=data["menu5"];
	...
	$.getAll();
  	...
  	$(document).ready(function(){
  		...
  		$(document.body).on('click', '#clientRegistrationWindow' ,function(){
  			$("#contentPanel").load("ClientCreation.jsp");
  		});
  		$(document.body).on('click', '#officerTypeWindow' ,function(){
  			$("#contentPanel").load("OfficerType.jsp");
  		});
  		$(document.body).on('click', '#OfficerWindow' ,function(){
  			$("#contentPanel").load("officer.jsp");
  		});
  		$(document.body).on('click', '#accessGroupWindow' ,function(){
  			$("#contentPanel").load("AccessGroups.jsp");
  		});
  		$(document.body).on('click', '#applicationsWindow' ,function(){
  			$("#contentPanel").load("Application.jsp");
  		});
  		$(document.body).on('click', '#applicationApprovalWindow' ,function(){
  			$("#contentPanel").load("Approval.jsp");
  		});  
